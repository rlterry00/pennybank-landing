...
var _mapPagesToStaticQueryHashes = require("../utils/map-pages-to-static-query-hashes");
  ...
  return new Promise(resolve => {
    compiler.hooks.done.tapAsync(`print gatsby instructions`, async function (stats, done) {
      ...
      if (isSuccessful) {
        ...
        const mapOfTemplatesToStaticQueryHashes = (0, _mapPagesToStaticQueryHashes.mapTemplatesToStaticQueryHashes)(state, stats.compilation);
        mapOfTemplatesToStaticQueryHashes.forEach((staticQueryHashes, componentPath) => {
          if (!(0, _lodash.isEqual)(state.staticQueriesByTemplate.get(componentPath), staticQueryHashes.map(String))) {
            store.dispatch({
